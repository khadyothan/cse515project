...
def findKimages(representative_image, feature_space, k):
    ...
    images_display = {image_id: {'image': image, 'distance': top_k_images[image_id]} for image_id, (image, label) in enumerate(dataset) if image_id in top_k_images}
    print_top_k_images.print_images(images_display, feature_space)
